...
1. configure the `HOMEBREW_TAP_GITHUB_TOKEN` secret with contents write access;
2. add Developer ID signing and Apple notarization for macOS artifacts;
3. verify execution on both macOS architectures without a Gatekeeper bypass;
4. test upgrade, rollback, and uninstall from a clean Homebrew prefix.
